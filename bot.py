...
CREATE TABLE IF NOT EXISTS panels (
    ...
    gamemode TEXT NOT NULL DEFAULT 'Queue',
...
panel_columns = {row["name"] for row in db.execute("PRAGMA table_info(panels)").fetchall()}
if "gamemode" not in panel_columns:
    db.execute("ALTER TABLE panels ADD COLUMN gamemode TEXT NOT NULL DEFAULT 'Queue'")
if "active" not in panel_columns:
    db.execute("ALTER TABLE panels ADD COLUMN active INTEGER NOT NULL DEFAULT 1")
db.commit()
...
def save_panel(guild_id, channel_id, message_id, gamemode):
    db.execute(
        "INSERT OR REPLACE INTO panels (guild_id, channel_id, message_id, gamemode, active) VALUES (?, ?, ?, ?, 1)",
        (str(guild_id), str(channel_id), str(message_id), gamemode),
    )
    db.commit()
...
def set_queue_active(guild_id, active):
    db.execute(
        "UPDATE panels SET active=? WHERE guild_id=?",
...
class QueueView(discord.ui.View):
    ...
    async def join_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not panel or not panel["active"]:
            await interaction.response.send_message(
                "⛔ Cette queue est actuellement arrêtée.",
                ephemeral=True,
            )
            return
...
async def ensure_first_ticket(guild: discord.Guild):
    ...
    if not rows or not panel or not panel["active"]:
        return
...
async def update_panel(guild: discord.Guild):
    ...
    gamemode = panel["gamemode"]

    embed = discord.Embed(
        title=f"🎟️ {gamemode} — Queue",
        description=(
            f"Queue **{gamemode}**\n\n"
            + (
                "🟢 **La queue est ouverte.**\n"
                "Clique sur **Rejoindre la queue** pour entrer.\n"
                "🥇 Quand tu arrives **1er**, un ticket privé est créé automatiquement."
                if active else
                "🔴 **La queue est arrêtée.** Impossible de rejoindre pour le moment."
            )
        ),
    ...
    if rows:
        embed.add_field(
            ...
            value=f"<@{rows[0]['user_id']}>",
            inline=False,
        )

    embed.set_footer(text=f"FranceTier • {gamemode}")
...
class QueueSetupModal(discord.ui.Modal, title="Créer une queue"):
    gamemode = discord.ui.TextInput(
        label="Nom du gamemode",
        placeholder="Exemple : Diamond FFA",
        min_length=1,
        max_length=80,
        required=True,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        if not interaction.guild or not isinstance(interaction.channel, discord.TextChannel):
            await interaction.response.send_message(
                "❌ Utilise cette commande dans un salon texte.",
                ephemeral=True,
            )
            return

        name = str(self.gamemode.value).strip()
        embed = discord.Embed(
            title=f"🎟️ {name} — Queue",
            description=(
                f"Bienvenue dans la queue **{name}** !\n\n"
                "🟢 **Rejoindre** → entre dans la queue\n"
                "🚪 **Quitter** → sort de la queue\n"
                "🥇 Quand tu es **1er**, un ticket privé est créé automatiquement.\n"
                "🔒 Le staff ferme le ticket quand le test est terminé."
            ),
            color=discord.Color.green(),
        )
        embed.add_field(name="👥 Queue", value="*La queue est vide.*", inline=False)
        embed.set_footer(text=f"FranceTier • {name}")

        message = await interaction.channel.send(embed=embed, view=QueueView())
        save_panel(interaction.guild.id, interaction.channel.id, message.id, name)
        await interaction.response.send_message(
            f"✅ Queue **{name}** créée !",
            ephemeral=True,
        )
...
@queue_group.command(name="setup", description="Créer une queue")
@app_commands.default_permissions(administrator=True)
async def queue_setup(interaction: discord.Interaction):
    await interaction.response.send_modal(QueueSetupModal())


@queue_group.command(name="stop", description="Arrêter la queue actuelle")
@app_commands.default_permissions(administrator=True)
async def queue_stop(interaction: discord.Interaction):
    ...
    if not panel["active"]:
        await interaction.response.send_message(
            f"⚠️ La queue **{panel['gamemode']}** est déjà arrêtée.",
            ephemeral=True,
        )
        return

    set_queue_active(interaction.guild.id, False)
    await update_panel(interaction.guild)
    await interaction.response.send_message(
        f"🛑 La queue **{panel['gamemode']}** est maintenant arrêtée. "
        "Les joueurs déjà présents restent dans la liste.",
        ephemeral=True,
    )
...
async def on_ready():
    ...
    # Copie les commandes globales dans chaque serveur pour qu'elles
    # apparaissent immédiatement dans Discord, sans attendre la propagation globale.
    for guild in bot.guilds:
        try:
            bot.tree.copy_global_to(guild=guild)
            synced = await bot.tree.sync(guild=guild)
            print(f"Commandes synchronisées sur {guild.name}: {len(synced)}")
            await ensure_first_ticket(guild)
            await update_panel(guild)
        except Exception as e:
            print(f"[ERREUR] Initialisation {guild.name}: {e}")

    # Synchronisation globale également, utile si le bot est ajouté à d'autres serveurs.
    try:
        await bot.tree.sync()
